...
subtitle: "Using Numpy Random Generator to Model Randomness"
...
Rate-based metrics are normally distributed; so we use the normal approximation to verify our simulation results are reasonable using with mean `{python} conversion_rate` and standard_error = `{python} f"({conversion_rate}"` * `{python} f"{1 - conversion_rate})/sqrt(10,000)"`.
