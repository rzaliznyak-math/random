---
title: "Use Numpy Random Generator to Model Randomness"
...
Our first simulation only generated `{python} control_simulated_results[0]` conversions. But with enough simulations plotted together, are you seeing a pattern develop?
...
Rate-based metrics are normally distributed; so we use the normal approximation to confirm our results with mean `{python} conversion_rate` and standard_error = `{python} standard_error`.
...
Differences are explained by the continuous nature of the normal approximation. Our data is discrete.


## Conclusion

Simulation studies will help us understand our problems when closed-form analytical solutions may not be readily available.
This simple example shows numpy.random can be used to simulate and understand the randomness of website conversion data.

In upcoming papers, we will extend these simulations to further our understanding of AB Testing concepts. Later we will model more complex processes like: Sequential & Bayesian Testing and the benefits of CUPED.
